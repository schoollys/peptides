...
- [x] ✅ PKI: **реальный** `EnvKeyPkiProvider` (`PKI_PROVIDER=env`) — Ed25519-ключ оракула берётся из секрета `PKI_PRIVATE_KEY` (бесплатно, без аккаунтов); ключ не в репозитории/БД и ротируется сменой env. Верификация COA не меняется (тот же Ed25519/`sources.pubkey`). Пути: реализация `lib/pki/ed25519.ts`, server-only реэкспорт `lib/pki/provider.ts`. Тулинг: `npm run pki:keygen` (генерация), `npm run pki:sync-oracle` (публикация публичного ключа в `sources.pubkey`), `npm run pki:selftest`. Прозрачность: `GET /api/pki/public-keys`. ⚠️ Опционально позже — managed cloud KMS/HSM (AWS `ECC_NIST_EDWARDS25519` / GCP `EC_SIGN_ED25519`) добавляется как ещё один `PKI_PROVIDER` без смены формата подписи; ротация/отзыв ключей оракулов.
- [x] ✅ Anchor: **реальный** `OtsAnchorProvider` (`ANCHOR_PROVIDER=ots`) — якорение хешей в Bitcoin через OpenTimestamps, бесплатно, без ключей/аккаунта. Двухфазно: запись пишет хеш как `pending` (без сети), а демон в кроне штампует/доращивает до подтверждения в блоке (`lib/anchor/ots-pipeline.ts`; хранилище доказательств — таблица `anchor_proofs`, миграция `0006`). По ADR-005 наружу уходит только опаковый хеш. Альтернатива — `ANCHOR_PROVIDER=http` (`HttpAnchorProvider`, реле/нотариус L2). CLI: `npm run anchor:ots`, smoke: `npm run ots:selftest`. Статус/номер BTC-блока + ссылка на mempool.space выведены в UI (`components/anchor/bitcoin-proof.tsx`: вкладка «Целостность» профиля и страница `/verify`). ⚠️ Опционально: merkle-батчинг сабмитов.
